...
# MAGIC - Gold    : conformite_dept, parametres_risks, commune_stats, evolution_mensuelle
...
def run_validation(context, df: pd.DataFrame, suite_name: str) -> dict:
    ...
    print(f"\n[{status}] {suite_name}")
    print(f"  Expectations : {total} total | {passed} passed | {total - passed} failed")
...
def add_suite_silver(ctx) -> None:
    ...
    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeInSet(
        ...
        value_set=["conforme", "non_conforme", "conforme_avec_remarque", "inconnu"]))
    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeInSet(
...
def add_suite_conformite_dept(ctx) -> None:
    ...
    suite.add_expectation(gx.expectations.ExpectColumnPairValuesAToBeGreaterThanB(
        column_A="nb_analyses", column_B="nb_conformes", or_equal=True))
...
def add_suite_evolution_mensuelle(ctx) -> None:
    suite = ctx.suites.add(gx.ExpectationSuite(name="gold_evolution_mensuelle"))
    suite.add_expectation(
        gx.expectations.ExpectTableRowCountToBeBetween(min_value=0))
...
def save_report_local(rows: list, ts: str) -> None:
    ...
    reports_dir = os.path.join(quality_cfg["local"], quality_cfg["reports_dir"])
...
def run_quality_checks():
    ...
    print("=" * 60)
    result_silver = run_validation(context, read_table("silver"), "silver_water_quality")

    print("\n" + "=" * 60)
    print("GOLD VALIDATION")
    print("=" * 60)
    result_conformite = run_validation(context, read_table("gold_conformite_dept"),    "gold_conformite_dept")
    result_risks      = run_validation(context, read_table("gold_parametres_risks"),   "gold_parametres_risks")
    result_communes   = run_validation(context, read_table("gold_commune_stats"),      "gold_commune_stats")
    result_evolution  = run_validation(context, read_table("gold_evolution_mensuelle"),"gold_evolution_mensuelle")
